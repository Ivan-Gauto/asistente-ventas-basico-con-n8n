...
if (!API_KEY) {
  console.error(
    '[n8nApi] Falta la variable de entorno VITE_N8N_API_KEY. ' +
...
export async function sendToN8nWebhook(message: string, sessionId: string, webhookUrl: string = '/webhook/mi-chat-bot') {
  const sanitizedUrl = webhookUrl.endsWith('/') ? webhookUrl.slice(0, -1) : webhookUrl;
  
  console.log(`[n8nApi] Realizando peticion POST a: ${sanitizedUrl}`);
  ...
  if (!res.ok) {
    ...
    if (res.status === 404) {
      throw new Error(`El Webhook no está activo o la ruta (${webhookUrl}) es incorrecta. Inícialo en "Test workflow" o actívalo en n8n.`);
    }
    throw new Error(errorData.message || `Error en la comunicacion con n8n (${res.status})`);
  }
